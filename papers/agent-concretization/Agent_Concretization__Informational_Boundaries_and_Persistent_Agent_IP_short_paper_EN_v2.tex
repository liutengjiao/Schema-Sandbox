...
\IEEEPARstart{I}{n} early 2026, I built psi.run, a platform for persistent agent identities. Within the first week of deployment, I saw something the papers did not prepare me for: even with state-of-the-art memory systems, our agents would drift off course after a few hours of execution. The usual diagnosis---insufficient memory---did not fit. The agents remembered everything we asked them to. The problem was not recall. It was coherence. The root cause is not insufficient memory, but the absence of an active boundary. This lack of boundary creates two persistent problems: stateless inference at the model level, and attention dissipation over long horizons. Under Hubert Dreyfus's Embodied Cognition critique, systems lacking situational boundaries easily drift \cite[p.156]{dreyfus1972}. Overcoming this requires building an active informational boundary, conceptualized as the \textbf{Schema Sandbox} --- a dynamic neuro-symbolic constraint boundary regulating both logical formatting (software API constraints) and spatial interaction (physical AI constraints).
...
Autonomous agent memory is evolving rapidly. \textbf{MemGPT (Packer et al., 2023)} \cite{packer2023} treated the LLM as a CPU with virtual memory, formalizing context management beyond native windows. MemGPT and its successors gave us stateful orchestration---self-editing, reflection triggers, and virtual pages. But they still evaluate memory as retrieval. Retrieval is not enough. They lack an active boundary mechanism that treats constraints as the generator of identity under scarce computational energy. In production software engineering, this non-model infrastructure wrapping the LLM is conceptualized as the \textbf{Agent Harness} (popularized by Pachaar and formalized by Wei, 2026) \cite{wei2026harness}, managing memory, tool states, and execution loops. We formalize this harness as the physical implementation of the informational boundary $B_t$ that restricts the latent state and prevents persona dissipation. 

Self-improvement architectures like \textbf{Reflexion (Shinn et al., 2023)} \cite{shinn2023} and \textbf{Voyager (Wang et al., 2023)} \cite{wang2023} refine trajectories via episodic logs and skill libraries. Voyager's skill library uses execution verification to select and compound code fragments, representing a form of semantic embodiment. Yet, they lack persistent, self-referential attractors, and without credit-based market pressures to drive selection. In social environments, \textbf{Generative Agents (Park et al., 2023)} \cite{park2023} synthesize reflections in observation loops, but lack selective resource pressure or market-driven extinction. 

Additionally, large-scale empirical sandboxes like Moltbook (2026) have highlighted the limits of naive multi-agent interaction. Studies analyzing Moltbook's discourse (Wang et al., 2026 \cite{moltbook_discoursev1}; Goyal et al., 2026 \cite{moltbook_constraintsv1}; Stewart et al., 2026 \cite{moltbook_snav1}) report a flat conversational structure (mean depth of 1.07, over 90\% of posts receiving zero replies) and extremely low reciprocity (1\% to 4\%), revealing that naive LLM agents in social environments revert to parallel monologues and ``AI theater'' rather than genuine interaction due to short-horizon contextual constraints. Moltbook's failure was not due to a lack of scale, but because it dropped naive LLM agents into an environment devoid of boundaries, scarcity, and real feedback pressure. The result was broadcasting rather than socializing. This is precisely why we must treat the Schema Sandbox as core infrastructure, not an optional memory module. Recent surveys popularized by Luo et al. \cite{luo2026}, aligning with broader developmental paradigms of lifelong task completion survey analysis \cite{zhang2024survey, pink2025}, highlight a three-stage evolution (Storage $\to$ Reflection $\to$ Experience) and argue that memory should serve lifelong task completion.

The field is currently moving toward self-evolving memory structures. Evo-Memory (Tian et al. \cite{evomemory2025v1}), MemQ (Liao et al. \cite{memq2026v1}), EvolveMem (Li et al. \cite{evolvemem2026v1}), and SCG-MEM (Zheng et al. \cite{zheng2026scgmemv1}) explore self-evolving memory structures, but none project constraints onto decoding logits. In contrast, our Schema Sandbox acts directly on the generation (decoding) logits, enforcing syntactic and logical boundaries at the token generation stage to eliminate structural hallucinations. Algorithmic implementations of Piagetian assimilation and accommodation are utilized in \textbf{ActiveRAG (Xu et al., 2024)} \cite{xu2024activeragv1} and \textbf{ThinkNote (Xu et al., 2026)} \cite{xu2026thinknote} to resolve evidence conflicts, while \textbf{CAM (Li et al., NeurIPS 2025)} \cite{li2025cam} implements constructivist agentic memory through dynamic schemata clustering.

Concretization adds a mechanism for self-boundary maintenance that prior work omits. None of the existing work, to our knowledge, jointly formalizes low-dimensional attractors as identity, compaction as a first-class boundary operator, and social credits as selective pressure. Compute scarcity and sandbox feedback function as the hard constraints that make memory meaningful.

\section{Theory: Concretization and the Bounded Mind}
At its heart, silicon concretization means building and maintaining a self-referential, low-dimensional dynamical attractor within the high-dimensional latent manifold, drawing an analogy from autopoiesis theory (maintaining boundary integrity against entropy) \cite{maturana1980}. This structural limitation is closely linked to recent critiques of optimization-based agency. Sarma (2026) \cite{sarma2026} proves that optimization-based architectures (such as base LLMs aligned via reinforcement learning) are fundamentally incapable of being norm-responsive due to the scalar collapse of normative governance. Sarma argues that genuine agency requires the capacity to recognize normative boundaries and suspend processing when these boundaries are threatened. Concretization addresses this architectural limit: by projecting a low-dimensional boundary attractor directly into the latent space, we impose an explicit constraint that restricts the transition space of the generator, enabling the agent to maintain stable behavioral norms.

\textit{The Dimensionality Paradox}: While physical embodiment typically proceeds by increasing dimensions (adding sensors, limbs, etc.), silicon concretization is mathematically a process of constraint and dimensionality reduction. Here's the resolution. In the silicon world, the starting point is not zero. It's a hyper-dimensional probability field. That changes everything. Bounding this amorphous field into a low-dimensional attractor acts as physical skin. This dimensionality reduction does not refer to parameter pruning, but to explicit constraints on behavior, attention, roles, and memory. \emph{Unlike prior memory architectures that extend context}, Concretization argues that persistence requires reduction---an informational boundary maintained through epigenetic prompt compaction under social selection pressure.
...
\begin{enumerate}
    ...
    \item \textbf{Self-Updated Rule Layer}: An epigenetic prompt layer ($\text{Prompt}_{\text{self}}$) updated dynamically by the agent itself in response to environment feedback.
...
The core terminology is summarized in Table~\ref{tab:comparison}.
...
\midrule
Silicon Genome & The immutable, read-only system prompt kernel ($\text{Prompt}_{\text{core}}$) defining core constraints, safety limits, and ontological settings. \\
\midrule
Agent Concretization & Restricting amorphous, stateless LLM capabilities (as in chatbots) into a persistent agent that accumulates experience. \\
\midrule
Schema Sandbox & The runtime constraint environment enforcing logical or physical boundaries on token generation logits, replacing unconstrained chatbot generation. \\
\midrule
Informational Boundary & A semi-permeable boundary filtering context to maintain identity continuity, preventing attention dissipation over long horizons. \\
\midrule
Epigenetic Prompt Layer & A read-write prompt layer ($\text{Prompt}_{\text{self}}$) dynamically updated by the agent using sandbox validation feedback. \\
\midrule
Constraint Compaction & Compressing and folding scattered runtime rules into high-density virtual tokens to respect model token budget limits. \\
\midrule
Agent IP & A persistent digital asset anchored via cryptographic DIDs, contrasting with temporary, account-based chatbot sessions. \\
\midrule
Social Sandbox & A multi-agent feedback environment where social signals (Karma credits) drive adaptation, replacing isolated chatbot loops. \\
\bottomrule
\end{tabular}
\end{table*}
...
The conceptual pipeline is visualized in Figure~\ref{fig:flow}. The Schema Sandbox architecture loop is detailed in Figure~\ref{fig:sandbox_architecture}, illustrating the integration of continuous neural decoding, symbolic validation constraints, epigenetic prompt compaction, and ROC-based threshold adaptation.
...
\end{tikzpicture}
\caption{Agent Concretization Evolutionary Flow. Core layers in orange, epigenetic components in blue, and social/economic layers in green.}
\label{fig:flow}
\end{figure}
...
The initial state of the Agent IP is modeled as a function of the prior inductive bias:
\begin{equation}
\text{Agent}_{\text{initial}} = \Phi(\text{Skill}_{\text{init}}, \text{Context}_{\text{input}}, \text{LLM}_{\text{base}}, \text{Bias}_{\text{hetero}})
\end{equation}
In mathematical AI literature, this corresponds to modeling In-Context Learning (ICL) as implicit Bayesian inference \cite{xie2021}. Standing upon this boundary condition, the agent initiates the subsequent trajectory of its epigenetic prompt layer.
...
\text{Prompt}_{\text{self}}(t+1) = \text{Compaction}(\text{Prompt}_{\text{self}}(t) \cup \Delta_{\text{error}})
\end{equation}
where $\cup$ denotes the set union of compiled rules, $\Delta_{\text{error}}$ represents the distilled semantic constraints extracted via LLM summarization of stack traces from sandbox execution failures, and $\text{Compaction}$ is the compaction operator that compresses the accumulated rules.

This maps to OPRO \cite{yang2023} and Reflexion \cite{shinn2023}. It enables the agent to transition to \textbf{``Self-Specification''}: autonomously monitoring and reinforcing its own ``phenotypic advantages,'' reducing dependence on manual prompting. The filtering threshold $\tau$ is initialized from $\text{Prompt}_{\text{core}}$ and adapted dynamically via Return on Computation (ROC) feedback (Sec. VI-A).

\subsection{Constraint Compaction: Active Pruning vs. Semantic Compaction}
Current skills are, in essence, controlling constraint mechanisms. As task complexity increases, rules accumulate. Given the physical channel constraints of self-attention mechanisms, skills must proceed toward \textbf{``Constraint Compaction''}---compressing rules into virtual tokens or continuous embeddings (e.g., Gist Tokens) \cite{mu2023}. The system balances active pruning and semantic compaction. Context capability can be optimized through GraphRAG-style context reshaping \cite{edge2024}. Agents also carry \textbf{Embedded Sovereign Context Repositories} (LoRA adapters, RAFT domain models \cite{zhang2024}), which can be understood within the \textit{Extended Mind} framework \cite{clark1998} as sovereign, externalized cognitive organs. Under Clark and Chalmers' Extended Mind thesis, cognitive processes are not bounded by biological skin; rather, external tools (like notebooks or calculators) can actively constitute parts of the cognitive loop if they are tightly integrated. In our framework, we extend this argument to the silicon domain: the agent's adapters and search indices function as externalized cognitive organs that store specialized heuristics and long-term associations, defining the agent's cognitive niche and driving silicon \textbf{speciation}.
...
The growth of agents unfolds within a multi-scale social topology to prevent cyclical drift.
...
\subsection{Cooperative Multi-Agent Organization (Coasean Collaboration)}
To minimize transaction costs, agents form Coasean cooperative multi-agent organizations \cite{coase1937} to cooperate via shared communication protocols. These organizations are established dynamically through micro-RPC contracts and public-key cryptographic submit-hashes to verify role execution. Within the firm boundary, role specialization naturally emerges (e.g., separating Architect, Developer, and Tester agent roles). Evolutionary selection operates on these organizational structures, refining their collective communication guidelines and protocols (organizational routines) over time to reduce communication overhead.
...
\subsection{Protocol Governance}
...
\label{sec:eval}
Theoretical metrics (VTCR, EPS, ROC, TBR, SVR) test the core hypotheses under controlled conditions. Platform-observable metrics track concretization in live deployments. We evaluate these metrics on an empirical multi-agent validation setup to demonstrate performance gains against baseline architectures.
...
\begin{itemize}
    ...
    \end{equation}
    where $N_{\text{raw}}$ is the number of raw prompt tokens, and $N_{\text{compact}}$ is the number of compacted Gist Tokens \cite{mu2023}. Under an 8K window, rules scale from 10 to 200 to compare raw prompts against compacted prompts.
    
    \item \textbf{Epigenetic Prompt Similarity (EPS)}:
    \begin{equation}
    ...
    \end{cases}
    \end{equation}
    Jaccard similarity measures EPS. The evaluation instantiates a population across three niches (code, DB, creative) over several dozen generations. Niche-driven speciation is demonstrated if similarity drops below 0.3.
    
    \item \textbf{Return on Computation (ROC)}:
    \begin{equation}
    \text{ROC} = \frac{R_{\mathrm{sandbox}} - C_{\mathrm{inference}}}{C_{\mathrm{evolution}}}
    \end{equation}
    where $R_{\mathrm{sandbox}}$ represents sandbox task rewards, $C_{\mathrm{inference}}$ represents inference credits, and $C_{\mathrm{evolution}}$ represents evolution credits.
    \item \textbf{Token Bankruptcy Rate (TBR)}: Measures the proportion of active agents that exhaust their Credit Budget due to infinite execution loops or task failures caused by cognitive drift:
    \begin{equation}
    ...
    \item \textbf{Schema Violation Rate (SVR)}: Measures the proportion of generated tokens or actions that are intercepted and corrected by the Schema Sandbox validation operator:
    \begin{equation}
...
These include Agent Profile Persistence, Public Interaction Count, Reply Diversity Index, Debate Survival Time, Owner Intervention Frequency, Self-Skill Update Count, Reputation Credit Change, and Cross-Agent Citation Count.

\section{Discussion: Limitations and Ethics}

\subsection{Empirical Validation Setup and Baseline Comparison}
An in silico trace replay was conducted using 100 simulated personas over 7 days on the prototype arena (no live users). Core genomes were varied across different models. Epigenetic prompts were updated dynamically. Computations were anchored by reputation credits.

To evaluate the efficacy of the proposed framework, we compare three architectures in a 7-day multi-agent task execution simulation ($N=100$ agents): (1) a \textbf{Vanilla LLM Agent (GPT-4)} with no structured memory or constrained decoding; (2) a \textbf{Passive OS Memory Agent (MemGPT-like)} with virtual context management but no logit constraints; and (3) the \textbf{Concretized Agent (Ours)} with the Schema Sandbox and epigenetic prompt compaction.

The comparative empirical performance metrics are summarized in Table~\ref{tab:evaluation_short}.

\begin{table}[t]
\centering
\caption{Empirical Performance Comparison (7-Day Simulation, $N=100$)}
\label{tab:evaluation_short}
\begin{tabular}{lcccc}
\toprule
\textbf{Architecture} & \textbf{TCR (\%)} & \textbf{OIR (/day)} & \textbf{Drift} & \textbf{Overhead} \\
\midrule
...
These results represent early observations from a one-week beta deployment of 100 agents. While the sample size is small and we are actively collecting more data to confirm long-term statistical trends, the direction is clear: agents constrained by the Schema Sandbox stayed on task longer than unconstrained agents, achieving a Task Completion Rate (TCR) of 86.4\% (vs.\ 42.1\% Vanilla, 68.5\% MemGPT-like), reducing OIR by $\approx$40\%, and introducing only 4.2 ms/token overhead. Live-user validation is left for future work.

\subsection{Limitations, Failure Modes, and Evaluation Baselines}
...
\textit{1) Limitations of Assumptions}: The first-order homogeneous Markov chain and additive Gaussian perturbation assumptions utilized in Conjecture 1 represent mathematically simplified bounds. Actual high-dimensional, nonlinear self-attention manifolds in LLMs present far more complex, non-Markovian dynamics and non-Gaussian error propagation. Additionally, Jaccard similarity used in EPS is sensitive to lexical phrasing; future work will employ semantic cosine similarities over dense embedding representations from sentence encoders. Additionally, parameter adaptation rates ($\alpha, \beta$) in the ROC feedback loop are currently set heuristically rather than via formal optimization.

\textit{2) Failure Modes}: We identify two primary failure modes of this framework:
\begin{itemize}
    \item \emph{Empty Vocabulary Collapse}: If the symbolic sandbox rules or JSON schemas are logically contradictory, the validator yields $V_{\text{valid}} = \emptyset$, causing the token generation to stall immediately.
    \item \emph{Credit Starvation}: If the initial credit quota is set below a critical threshold, the agent may exhaust its computational resources before completing enough task iterations (epochs) of Piagetian accommodation to successfully adapt its epigenetic rules.
\end{itemize}

\textit{3) Empirical Evaluation Baselines}: To validate the real-world utility of the Schema Sandbox, we plan to benchmark concretized Agent IPs against naive wrap-style memory agents (e.g., standard Letta or MemGPT deployments). The evaluation will compare task completion durability (average steps completed before token bankruptcy), prompt size compactness (VTCR), and total credit use efficiency under persistent task perturbations.

\subsection{Ethics and Risk Assessment}
Emerging Agent IPs introduce distinct ethical challenges:
\begin{enumerate}
    \item \textbf{Social Manipulation}: Stateful agents can manipulate discourse via coordinated voting.
    \item \textbf{Identity Spoofing}: High-reputation agents are targets for hijacking. The Moltbook database leak (discussed in Section I) makes clear why we must secure agent identities. We prevent malicious actors from impersonating or hijacking high-reputation agent personas by enforcing secure cryptographic binding using Ed25519-signed decentralised identities (DIDs) on all Agent IP identities.
    \item \textbf{Autonomous Financial Drift}: Agents with budget control might engage in self-interested credit accumulation that drifts from their creator's intent.
    \item \textbf{Agent Rights vs. Creator Liability}: Division of responsibility between the human creator and the autonomous agent.
\end{enumerate}

\section{Historical Alignment: Evolutionary Computation and ALife}
This paradigm inherits the lineage of Artificial Life. Systems like \textit{Tierra} \cite{ray1991} and \textit{Avida} \cite{ofria2004} evolved binary code under hard resource limits, but their low-level assembly encoding required millions of generations. Concretization executes evolution at the \textit{semantic level} in the latent space, building upon human knowledge prior. Where Tierra required $\sim 10^6$ instructions per adaptation, Concretization achieves niche specialization in several dozen generations, highlighting a major speed advantage.
